...
void j1Player::Start_F3()
{
	...
	//Restart player vars
	player.player_flip = App->config_file.child("player_info").attribute("flip").as_bool();
	player.player_not_jumping = App->config_file.child("player_info").attribute("not_jumping").as_bool();
	player.player_god_mode = App->config_file.child("player_info").attribute("god_mode").as_bool();
	player.player_tang_mode = App->config_file.child("player_info").attribute("tang_mode").as_bool();
	player.player_alive = App->config_file.child("player_info").attribute("alive").as_bool();
	player.player_respawn = App->config_file.child("player_info").attribute("respawn").as_bool();
	player.player_climbing = App->config_file.child("player_info").attribute("climbing").as_bool();
	player.spacebar_pushed = App->config_file.child("player_info").attribute("spacebar_pushed").as_bool();
	inputs_out = App->config_file.child("inputs_out").attribute("value").as_int();
	actual_state = (state)App->config_file.child("actual_state").attribute("value").as_int();
