...
void j1App::Load() {
	...
	if (save_node == NULL)
		LOG("Error in loading node.");
	for (int i = 0; i < modules.count(); i++) {
		for (pugi::xml_node_iterator iterator = save_node.begin(); iterator != save_node.end(); iterator++) {
			if (!strcmp(iterator->name(), modules[i]->name.GetString())) {
				pugi::xml_node node_sent = save_node.child(iterator->name());
				modules[i]->Load(node_sent);
			}
		}
	}
...
void j1App::Save() {
	...
	pugi::xml_node save_node = document_save.append_child("save");
	for (int i = 0; i < modules.count(); i++) {
		pugi::xml_node node = save_node.append_child(modules[i]->name.GetString());
		modules[i]->Save(node);
	}
	document_save.save_file("savegame.xml");
}
