...
bool j1App::Awake()
{
	...
	while(item != NULL && ret == true)
	{
		...
		for (pugi::xml_node_iterator iterator = node.begin(); iterator != node.end(); iterator++) {
			LOG("%s\n\n\n\n", iterator->name());
			if (!strcmp(iterator->name(), item->data->name.GetString())) {
				LOG("YES: %s; %s", iterator->name(), item->data->name.GetString());
				node_pointer = iterator->first_child();
				ret = item->data->Awake(&node_pointer.parent());
				node_found = true;
			}
			else {
				LOG("NO: %s; %s", iterator->name(), item->data->name.GetString());
			}
		}
		if (node_found == false)ret = item->data->Awake(nullptr);
		item = item->next; 
	}
